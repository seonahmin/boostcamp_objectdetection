...
def train(cfg, model, best_AP50, resume=True):
    ...
    with EventStorage(start_iter) as storage:
        for data, iteration in zip(data_loader, range(start_iter, max_iter)):
            ...
            wandb.log({'loss_cls':loss_dict_reduced['loss_cls'], 'loss_box_reg' : loss_dict_reduced['loss_box_reg']})
            if comm.is_main_process():
...
def setup(args):
    ...
    cfg.SOLVER.MAX_ITER = 25000 # 20 epoch
    cfg.SOLVER.STEPS = (24000,)
    cfg.SOLVER.GAMMA = 0.1
    cfg.SOLVER.CHECKPOINT_PERIOD = 5000
    cfg.OUTPUT_DIR = os.path.join('./detectron2/output/','_'.join([cfg.MODEL.META_ARCHITECTURE,cfg.SOLVER.OPTIMIZER, str(cfg.SOLVER.BASE_LR), str(cfg.SOLVER.GAMMA)]))
    ...
    cfg.freeze()
    wandb.init(name='_'.join(['model',cfg.MODEL.META_ARCHITECTURE,'optim',cfg.SOLVER.OPTIMIZER,'lr',str(cfg.SOLVER.BASE_LR), 'GAMMA', str(cfg.SOLVER.GAMMA)]), config=cfg)
    return cfg
