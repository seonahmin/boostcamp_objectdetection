...
from detectron2.solver import build_lr_scheduler, build_optimizer, get_default_optimizer_params
...
def do_train(cfg, model, best_AP50, resume=True):
    model.train()
    
    optimizer = AdamW(get_default_optimizer_params(model, weight_decay_norm=0), lr=cfg.SOLVER.BASE_LR)
...
def setup(args):
    ...
    """
    cfg = get_cfg()
    cfg.merge_from_file(model_zoo.get_config_file(args.config_file)) #args.config_file = COCO-Detection/faster_rcnn_R_101_FPN_3x.yaml
...
def main(args):
    ...
    best_AP50 = 0
    do_train(cfg, model, best_AP50, resume=True)
    return do_test(cfg, model)
