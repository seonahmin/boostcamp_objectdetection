...
def parse_args():
    ...
    parser.add_argument('--pseudo', action='store_true', help='whether use pseudo-labeling')
    ...
    parser.add_argument('--fold', type=int, default=3, help='if no kfold cross validation, you must set fold number')
...
def main(fold, args):
    ...
    if args.no_validate: # 즉, no_validate를 하게 되면 validate안쓰면 무조건 all data로 학습시킨다는 의미.
        ...
        if args.pseudo:
            ...
            conf_thrs = 30
            json_name = f'test_{csv_file}_conf{conf_thrs}'
            cfg.data.train.ann_file = cfg.json_root + f'train_{fold}_{json_name}.json' 
    ...
    # wandb kfold setting
    cfg.log_config['hooks'][1]['init_kwargs']['project'] = 'object_detection_seonah'
    cfg.log_config['hooks'][1]['init_kwargs']['entity'] = 'mg_generation'
    cfg.log_config['hooks'][1]['init_kwargs']['group'] = workdir.split('/')[-1]
    cfg.log_config['hooks'][1]['init_kwargs']['job_type'] = f'Fold{fold}'
    cfg.log_config['hooks'][1]['init_kwargs']['name'] = workdir.split('/')[-1] + f'_Fold{fold}'
    ...
    # cfg.model.rpn_head.anchor_generator.scales = [8]
    # cfg.model.rpn_head.anchor_generator.ratios = [0.33, 0.5, 1.0, 2.0, 3.0]
